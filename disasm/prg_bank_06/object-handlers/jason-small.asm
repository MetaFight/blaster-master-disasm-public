.macro MAC_L_815C
; ----------------------------------------------------------------------------
L_815C: jmp     L_848F                          ; 815C

; ----------------------------------------------------------------------------
L_815F: lda     #$40                            ; 815F
        sta     LoadedObj_CollisionBox_HalfWidth; 8161
        lda     #$80                            ; 8163
        sta     LoadedObj_CollisionBox_HalfHeight; 8165
        lda     LoadedObj + Obj::Health         ; 8167
        bne     L_817A                          ; 8169
        lda     LoadedObj + Obj::Scratch0       ; 816B
        cmp     #$01                            ; 816D
        bne     L_8177                          ; 816F
        lda     #$04                            ; 8171
        sta     $4F                             ; 8173
        bne     L_817A                          ; 8175
L_8177: jmp     L_848F                          ; 8177

; ----------------------------------------------------------------------------
L_817A: lda     LoadedObj + Obj::Health         ; 817A
        beq     L_819A                          ; 817C
        lda     $F7                             ; 817E
        and     #$03                            ; 8180
        cmp     #$01                            ; 8182
        beq     L_8196                          ; 8184
        cmp     #$02                            ; 8186
        beq     L_8192                          ; 8188
        lda     LoadedObj + Obj::Facing         ; 818A
        ora     #$40                            ; 818C
        sta     LoadedObj + Obj::Facing         ; 818E
        bne     L_819A                          ; 8190
L_8192: lda     #$80                            ; 8192
        bne     L_8198                          ; 8194
L_8196: lda     #$00                            ; 8196
L_8198: sta     LoadedObj + Obj::Facing         ; 8198
L_819A: lda     LoadedObj + Obj::Scratch0       ; 819A
        beq     L_81BA                          ; 819C
        cmp     #$01                            ; 819E
        beq     L_81E1                          ; 81A0
        cmp     #$02                            ; 81A2
        bne     L_81A9                          ; 81A4
        jmp     L_8246                          ; 81A6

; ----------------------------------------------------------------------------
L_81A9: cmp     #$03                            ; 81A9
        bne     L_81B0                          ; 81AB
        jmp     L_834D                          ; 81AD

; ----------------------------------------------------------------------------
L_81B0: cmp     #$04                            ; 81B0
        bne     L_81B7                          ; 81B2
        jmp     L_83BD                          ; 81B4

; ----------------------------------------------------------------------------
L_81B7: jmp     L_848F                          ; 81B7

; ----------------------------------------------------------------------------
L_81BA: bit     LoadedObj + Obj::Facing         ; 81BA
        bvs     L_81CB                          ; 81BC
        jsr     L_96A2                          ; 81BE
        lda     $D9                             ; 81C1
        bne     L_81DB                          ; 81C3
        bit     LoadedObj + Obj::Facing         ; 81C5
        bmi     L_81E9                          ; 81C7
        bpl     L_81F0                          ; 81C9
L_81CB: lda     $D9                             ; 81CB
        bne     L_81F4                          ; 81CD
        lda     LoadedObj + Obj::Scratch0       ; 81CF
        bne     L_81F4                          ; 81D1
        lda     LoadedObj + Obj::Velocity_X     ; 81D3
        beq     L_81F4                          ; 81D5
        bpl     L_81EB                          ; 81D7
        bmi     L_81F2                          ; 81D9
L_81DB: lda     $10                             ; 81DB
        and     #$03                            ; 81DD
        bne     L_81F4                          ; 81DF
L_81E1: bit     LoadedObj + Obj::Facing         ; 81E1
        bvs     L_81F4                          ; 81E3
        bpl     L_81F2                          ; 81E5
        bmi     L_81EB                          ; 81E7
L_81E9: dec     LoadedObj + Obj::Velocity_X     ; 81E9
L_81EB: dec     LoadedObj + Obj::Velocity_X     ; 81EB
        jmp     L_81F4                          ; 81ED

; ----------------------------------------------------------------------------
L_81F0: inc     LoadedObj + Obj::Velocity_X     ; 81F0
L_81F2: inc     LoadedObj + Obj::Velocity_X     ; 81F2
L_81F4: lda     LoadedObj + Obj::Scratch0       ; 81F4
        bne     L_820A                          ; 81F6
        lda     $F3                             ; 81F8
        bpl     L_820A                          ; 81FA
        lda     L855E                           ; 81FC
        sta     LoadedObj + Obj::Velocity_Y     ; 81FF
        lda     #$01                            ; 8201
        sta     LoadedObj + Obj::Scratch0       ; 8203
        lda     #$3B                            ; 8205
        jsr     Enqueue_Sound_Command           ; 8207
L_820A: jsr     LD2AB                           ; 820A
        and     #$40                            ; 820D
        beq     L_8215                          ; 820F
        lda     #$08                            ; 8211
        bne     L_8217                          ; 8213
L_8215: lda     #$10                            ; 8215
L_8217: ldx     #$4C                            ; 8217
        jsr     Speed_Limit_Sub                 ; 8219
        jsr     LD37A                           ; 821C
        beq     L_8243                          ; 821F
        lda     LoadedObj + Obj::Scratch0       ; 8221
        bne     L_823F                          ; 8223
        bit     LoadedObj + Obj::Facing         ; 8225
        bvs     L_823F                          ; 8227
        lda     #$01                            ; 8229
        bit     LoadedObj + Obj::Velocity_X     ; 822B
        bpl     L_8231                          ; 822D
        lda     #$FF                            ; 822F
L_8231: jsr     LD2B1                           ; 8231
        jsr     LD238                           ; 8234
        bne     L_823F                          ; 8237
        jsr     L_86EE                          ; 8239
        jmp     L_848F                          ; 823C

; ----------------------------------------------------------------------------
L_823F: lda     #$00                            ; 823F
        sta     LoadedObj + Obj::Velocity_X     ; 8241
L_8243: jmp     L_826F                          ; 8243

; ----------------------------------------------------------------------------
L_8246: lda     #$80                            ; 8246
        sta     LoadedObj_CollisionBox_HalfWidth; 8248
        lda     #$40                            ; 824A
        sta     LoadedObj_CollisionBox_HalfHeight; 824C
        lda     $F3                             ; 824E
        and     #$08                            ; 8250
        beq     L_825C                          ; 8252
        lda     #$00                            ; 8254
        sta     LoadedObj + Obj::Scratch0       ; 8256
        lda     #$80                            ; 8258
        sta     LoadedObj + Obj::Position_Y_Lo  ; 825A
L_825C: lda     #$00                            ; 825C
        bit     LoadedObj + Obj::Facing         ; 825E
        bvs     L_826A                          ; 8260
        bpl     L_8268                          ; 8262
        lda     #$FC                            ; 8264
        bne     L_826A                          ; 8266
L_8268: lda     #$04                            ; 8268
L_826A: sta     LoadedObj + Obj::Velocity_X     ; 826A
        jsr     LD37A                           ; 826C
L_826F: inc     LoadedObj + Obj::Velocity_Y     ; 826F
        inc     LoadedObj + Obj::Velocity_Y     ; 8271
        lda     #$40                            ; 8273
        ldx     #$4D                            ; 8275
        jsr     Speed_Limit_Sub                 ; 8277
        jsr     LD3DE                           ; 827A
        bne     L_82A1                          ; 827D
        jsr     LD2AB                           ; 827F
        and     #$40                            ; 8282
        bne     L_828A                          ; 8284
        lda     #$01                            ; 8286
        bne     L_828C                          ; 8288
L_828A: lda     #$04                            ; 828A
L_828C: sta     LoadedObj + Obj::Scratch0       ; 828C
        lda     $F3                             ; 828E
        bpl     L_829E                          ; 8290
        jsr     LD2AB                           ; 8292
        jsr     LD283                           ; 8295
        bne     L_829E                          ; 8298
        lda     #$03                            ; 829A
        sta     LoadedObj + Obj::Scratch0       ; 829C
L_829E: jmp     L_848C                          ; 829E

; ----------------------------------------------------------------------------
L_82A1: lda     LoadedObj + Obj::Velocity_Y     ; 82A1
        bpl     L_82AC                          ; 82A3
        lda     #$00                            ; 82A5
        sta     LoadedObj + Obj::Velocity_Y     ; 82A7
        jmp     L_848C                          ; 82A9

; ----------------------------------------------------------------------------
L_82AC: lda     LoadedObj + Obj::Scratch0       ; 82AC
        cmp     #$01                            ; 82AE
        bne     L_82B4                          ; 82B0
        dec     LoadedObj + Obj::Scratch0       ; 82B2
L_82B4: lda     LoadedObj + Obj::Velocity_Y     ; 82B4
        cmp     #$32                            ; 82B6
        bcs     L_82C0                          ; 82B8
        lda     #$00                            ; 82BA
        sta     LoadedObj + Obj::Velocity_Y     ; 82BC
        beq     L_82E4                          ; 82BE
L_82C0: lda     #$01                            ; 82C0
        sta     LoadedObj + Obj::Scratch0       ; 82C2
        lda     #$08                            ; 82C4
        sta     $4F                             ; 82C6
        lda     LoadedObj + Obj::Velocity_Y     ; 82C8
        cmp     #$40                            ; 82CA
        bcs     L_82D5                          ; 82CC
        lda     LoadedObj + Obj::Health         ; 82CE
        sec                                     ; 82D0
        sbc     #$40                            ; 82D1
        bcs     L_82D7                          ; 82D3
L_82D5: lda     #$00                            ; 82D5
L_82D7: sta     LoadedObj + Obj::Health         ; 82D7
        lda     #$E0                            ; 82D9
        sta     LoadedObj + Obj::Velocity_Y     ; 82DB
        lda     LoadedObj + Obj::Health         ; 82DD
        bne     L_82E4                          ; 82DF
        jmp     L_834A                          ; 82E1

; ----------------------------------------------------------------------------
L_82E4: lda     $F3                             ; 82E4
        and     #$04                            ; 82E6
        beq     L_831B                          ; 82E8
        ldx     LoadedObj + Obj::TileIndex      ; 82EA
        lda     $0500,x                         ; 82EC
        jsr     LD238                           ; 82EF
        bne     L_82FC                          ; 82F2
        lda     #$01                            ; 82F4
        jsr     LD1EF                           ; 82F6
        jmp     L_8347                          ; 82F9

; ----------------------------------------------------------------------------
L_82FC: txa                                     ; 82FC
        clc                                     ; 82FD
        adc     #$11                            ; 82FE
        tax                                     ; 8300
        lda     $0500,x                         ; 8301
        jsr     LD283                           ; 8304
        beq     L_8314                          ; 8307
        lda     #$02                            ; 8309
        sta     LoadedObj + Obj::Scratch0       ; 830B
        lda     #$C0                            ; 830D
        sta     LoadedObj + Obj::Position_Y_Lo  ; 830F
        jmp     L_834A                          ; 8311

; ----------------------------------------------------------------------------
L_8314: lda     #$03                            ; 8314
        sta     LoadedObj + Obj::Scratch0       ; 8316
        jmp     L_834A                          ; 8318

; ----------------------------------------------------------------------------
L_831B: lda     $F3                             ; 831B
        and     #$08                            ; 831D
        beq     L_8347                          ; 831F
        lda     #$EF                            ; 8321
        jsr     LD2B1                           ; 8323
        jsr     LD283                           ; 8326
        bne     L_8332                          ; 8329
        lda     #$03                            ; 832B
        sta     LoadedObj + Obj::Scratch0       ; 832D
        jmp     L_834A                          ; 832F

; ----------------------------------------------------------------------------
L_8332: jsr     LD2AB                           ; 8332
        and     #$40                            ; 8335
        bne     L_833C                          ; 8337
        jmp     L_834A                          ; 8339

; ----------------------------------------------------------------------------
L_833C: lda     #$F0                            ; 833C
        sta     LoadedObj + Obj::Velocity_Y     ; 833E
        lda     #$04                            ; 8340
        sta     LoadedObj + Obj::Scratch0       ; 8342
        jmp     L_834A                          ; 8344

; ----------------------------------------------------------------------------
L_8347: jsr     L_8624                          ; 8347
L_834A: jmp     L_848C                          ; 834A

; ----------------------------------------------------------------------------
L_834D: lda     #$80                            ; 834D
        sta     LoadedObj + Obj::Position_X_Lo  ; 834F
        lda     #$00                            ; 8351
        sta     LoadedObj + Obj::Velocity_X     ; 8353
        jsr     LD2AB                           ; 8355
        jsr     LD283                           ; 8358
        beq     L_8363                          ; 835B
        lda     #$01                            ; 835D
        sta     LoadedObj + Obj::Scratch0       ; 835F
        bne     L_83AE                          ; 8361
L_8363: ldx     #$00                            ; 8363
        lda     $F7                             ; 8365
        and     #$0C                            ; 8367
        beq     L_8373                          ; 8369
        ldx     #$08                            ; 836B
        cmp     #$04                            ; 836D
        beq     L_8373                          ; 836F
        ldx     #$F8                            ; 8371
L_8373: stx     LoadedObj + Obj::Velocity_Y     ; 8373
        jsr     LD2FE                           ; 8375
        lda     LoadedObj + Obj::Velocity_Y     ; 8378
        beq     L_83AE                          ; 837A
        bmi     L_8390                          ; 837C
        lda     LoadedObj + Obj::Position_Y_Lo  ; 837E
        bpl     L_83AE                          ; 8380
        lda     #$11                            ; 8382
        jsr     LD2B1                           ; 8384
        bpl     L_83AE                          ; 8387
        jsr     LD283                           ; 8389
        beq     L_83AE                          ; 838C
        bne     L_83A4                          ; 838E
L_8390: lda     LoadedObj + Obj::Position_Y_Lo  ; 8390
        bpl     L_83AE                          ; 8392
        cmp     #$90                            ; 8394
        bcs     L_83AE                          ; 8396
        lda     #$11                            ; 8398
        jsr     LD2B1                           ; 839A
        bpl     L_83AE                          ; 839D
        jsr     LD283                           ; 839F
        bne     L_83AE                          ; 83A2
L_83A4: lda     #$80                            ; 83A4
        sta     LoadedObj + Obj::Position_Y_Lo  ; 83A6
        lda     #$00                            ; 83A8
        sta     LoadedObj + Obj::Scratch0       ; 83AA
        bne     L_83AE                          ; 83AC
L_83AE: lda     $F3                             ; 83AE
        bpl     L_83BA                          ; 83B0
        lda     #$01                            ; 83B2
        sta     LoadedObj + Obj::Scratch0       ; 83B4
        lda     #$E0                            ; 83B6
        sta     LoadedObj + Obj::Velocity_Y     ; 83B8
L_83BA: jmp     L_848F                          ; 83BA

; ----------------------------------------------------------------------------
L_83BD: lda     #$40                            ; 83BD
        sta     LoadedObj_CollisionBox_HalfWidth; 83BF
        lda     #$40                            ; 83C1
        sta     LoadedObj_CollisionBox_HalfHeight; 83C3
        bit     LoadedObj + Obj::Facing         ; 83C5
        bvs     L_83CD                          ; 83C7
        bmi     L_83D8                          ; 83C9
        bpl     L_83DD                          ; 83CB
L_83CD: lda     $10                             ; 83CD
        lsr     a                               ; 83CF
        bcs     L_83DF                          ; 83D0
        lda     LoadedObj + Obj::Velocity_X     ; 83D2
        beq     L_83DF                          ; 83D4
        bmi     L_83DD                          ; 83D6
L_83D8: dec     LoadedObj + Obj::Velocity_X     ; 83D8
        jmp     L_83DF                          ; 83DA

; ----------------------------------------------------------------------------
L_83DD: inc     LoadedObj + Obj::Velocity_X     ; 83DD
L_83DF: lda     #$10                            ; 83DF
        ldx     #$4C                            ; 83E1
        jsr     Speed_Limit_Sub                 ; 83E3
        jsr     LD37A                           ; 83E6
        beq     L_841B                          ; 83E9
        lda     LoadedObj + Obj::Position_Y_Lo  ; 83EB
        cmp     #$41                            ; 83ED
        bcs     L_841B                          ; 83EF
        lda     #$F0                            ; 83F1
        bit     LoadedObj + Obj::Velocity_X     ; 83F3
        bpl     L_83F9                          ; 83F5
        lda     #$EE                            ; 83F7
L_83F9: jsr     LD2B1                           ; 83F9
        and     #$C0                            ; 83FC
        bne     L_841B                          ; 83FE
        lda     #$00                            ; 8400
        sta     LoadedObj + Obj::Scratch0       ; 8402
        lda     #$80                            ; 8404
        sta     LoadedObj + Obj::Position_Y_Lo  ; 8406
        dec     LoadedObj + Obj::Position_Y_Hi  ; 8408
        lda     LoadedObj + Obj::TileIndex      ; 840A
        sec                                     ; 840C
        sbc     #$11                            ; 840D
        sta     LoadedObj + Obj::TileIndex      ; 840F
        lda     #$E0                            ; 8411
        bit     LoadedObj + Obj::Velocity_X     ; 8413
        bpl     L_8419                          ; 8415
        lda     #$20                            ; 8417
L_8419: sta     LoadedObj + Obj::Position_X_Lo  ; 8419
L_841B: lda     $F7                             ; 841B
        and     #$0C                            ; 841D
        cmp     #$04                            ; 841F
        beq     L_8434                          ; 8421
        cmp     #$08                            ; 8423
        beq     L_842F                          ; 8425
        lda     LoadedObj + Obj::Velocity_Y     ; 8427
        cmp     #$10                            ; 8429
        beq     L_8436                          ; 842B
        bmi     L_8434                          ; 842D
L_842F: dec     LoadedObj + Obj::Velocity_Y     ; 842F
        jmp     L_8436                          ; 8431

; ----------------------------------------------------------------------------
L_8434: inc     LoadedObj + Obj::Velocity_Y     ; 8434
L_8436: lda     LoadedObj + Obj::Velocity_Y     ; 8436
        bmi     L_8444                          ; 8438
        cmp     #$20                            ; 843A
        bcc     L_844C                          ; 843C
        lda     #$20                            ; 843E
        sta     LoadedObj + Obj::Velocity_Y     ; 8440
        bne     L_844C                          ; 8442
L_8444: cmp     #$F0                            ; 8444
        bcs     L_844C                          ; 8446
        lda     #$F0                            ; 8448
        sta     LoadedObj + Obj::Velocity_Y     ; 844A
L_844C: jsr     LD3DE                           ; 844C
        beq     L_8465                          ; 844F
        lda     LoadedObj + Obj::Velocity_Y     ; 8451
        bmi     L_845F                          ; 8453
        lda     #$80                            ; 8455
        sta     LoadedObj + Obj::Position_Y_Lo  ; 8457
        lda     #$00                            ; 8459
        sta     LoadedObj + Obj::Scratch0       ; 845B
        beq     L_847C                          ; 845D
L_845F: lda     #$10                            ; 845F
        sta     LoadedObj + Obj::Velocity_Y     ; 8461
        bne     L_847C                          ; 8463
L_8465: lda     LoadedObj + Obj::Position_Y_Lo  ; 8465
        cmp     #$40                            ; 8467
        bcs     L_847C                          ; 8469
        lda     #$EF                            ; 846B
        jsr     LD2B1                           ; 846D
        and     #$40                            ; 8470
        bne     L_847C                          ; 8472
        lda     #$40                            ; 8474
        sta     LoadedObj + Obj::Position_Y_Lo  ; 8476
        lda     #$08                            ; 8478
        sta     LoadedObj + Obj::Velocity_Y     ; 847A
L_847C: lda     $F3                             ; 847C
        bpl     L_848C                          ; 847E
        jsr     LD2AB                           ; 8480
        jsr     LD283                           ; 8483
        bne     L_848C                          ; 8486
        lda     #$03                            ; 8488
        sta     LoadedObj + Obj::Scratch0       ; 848A
L_848C: jsr     L_880B                          ; 848C
L_848F: lda     #$08                            ; 848F
        sta     $40                             ; 8491
        lda     #$10                            ; 8493
        sta     $41                             ; 8495
        jsr     LCBDF                           ; 8497
        jsr     ScreenPos_Compute               ; 849A
        jsr     L_85E2                          ; 849D
        lda     LoadedObj + Obj::Health         ; 84A0
        bne     L_84B6                          ; 84A2
        lda     $4F                             ; 84A4
        bne     L_84B6                          ; 84A6
L_84A8: lda     #$1C                            ; 84A8
        sta     LoadedObj + Obj::Type           ; 84AA
        lda     #$00                            ; 84AC
        sta     LoadedObj + Obj::Scratch1       ; 84AE
        lda     $C5                             ; 84B0
        ora     #$08                            ; 84B2
        sta     $C5                             ; 84B4
L_84B6: lda     LoadedObj + Obj::Facing         ; 84B6
        eor     #$80                            ; 84B8
        and     #$80                            ; 84BA
        lsr     a                               ; 84BC
        sta     $44                             ; 84BD
        ldx     LoadedObj + Obj::Scratch0       ; 84BF
        beq     L_84DD                          ; 84C1
        dex                                     ; 84C3
        beq     L_8504                          ; 84C4
        dex                                     ; 84C6
        beq     L_8508                          ; 84C7
        dex                                     ; 84C9
        beq     L_84CF                          ; 84CA
        jmp     L_851B                          ; 84CC

; ----------------------------------------------------------------------------
L_84CF: ldx     #$00                            ; 84CF
        lda     LoadedObj + Obj::Position_Y_Lo  ; 84D1
        bpl     L_84D7                          ; 84D3
        ldx     #$40                            ; 84D5
L_84D7: stx     $44                             ; 84D7
        lda     #$67                            ; 84D9
        bne     L_8515                          ; 84DB
L_84DD: lda     LoadedObj + Obj::Velocity_X     ; 84DD
        beq     L_8500                          ; 84DF
        lda     $D9                             ; 84E1
        beq     L_84EB                          ; 84E3
        lda     $F7                             ; 84E5
        and     #$03                            ; 84E7
        beq     L_8500                          ; 84E9
L_84EB: lda     $1C                             ; 84EB
        rol     a                               ; 84ED
        rol     a                               ; 84EE
        rol     a                               ; 84EF
        sta     $45                             ; 84F0
        lda     $3E                             ; 84F2
        lsr     a                               ; 84F4
        lsr     a                               ; 84F5
        clc                                     ; 84F6
        adc     $45                             ; 84F7
        and     #$03                            ; 84F9
        clc                                     ; 84FB
        adc     #$61                            ; 84FC
        bne     L_8515                          ; 84FE
L_8500: lda     #$60                            ; 8500
        bne     L_8515                          ; 8502
L_8504: lda     #$61                            ; 8504
        bne     L_8515                          ; 8506
L_8508: lda     LoadedObj + Obj::Position_X_Lo  ; 8508
        rol     a                               ; 850A
        rol     a                               ; 850B
        rol     a                               ; 850C
        and     #$01                            ; 850D
        clc                                     ; 850F
        adc     #$65                            ; 8510
        jmp     L_8515                          ; 8512

; ----------------------------------------------------------------------------
L_8515: jsr     MetaSprite_Render                           ; 8515
        jmp     L_88E9                          ; 8518

; ----------------------------------------------------------------------------
L_851B: lda     L_8540                          ; 851B
        sta     DispatchPtr                     ; 851E
        lda     L_8540+1                        ; 8520
        sta     DispatchPtrHi                   ; 8523
        ldy     #$00                            ; 8525
        ldx     #$20                            ; 8527
        lda     $F7                             ; 8529
        and     #$0F                            ; 852B
        beq     L_8531                          ; 852D
        ldx     #$08                            ; 852F
L_8531: txa                                     ; 8531
        ldx     #$00                            ; 8532
        and     $10                             ; 8534
        beq     L_8539                          ; 8536
        inx                                     ; 8538
L_8539: txa                                     ; 8539
        jsr     LF029                           ; 853A
        jmp     L_88E9                          ; 853D

; ----------------------------------------------------------------------------
L_8540: .addr   L_8542                          ; 8540
L_8542: .addr   L_8546                          ; 8542
        .addr   L_8552                          ; 8544
; ----------------------------------------------------------------------------
L_8546: .byte   $FC,$00,$00,$88,$08,$00,$00,$98 ; 8546
        .byte   $F8,$F8,$10,$8A                 ; 854E
L_8552: .byte   $FC,$00,$00,$89,$08,$00,$00,$99 ; 8552
        .byte   $F8,$F8,$10,$8A                 ; 855A
L855E:  .byte   $D0                             ; 855E
; ----------------------------------------------------------------------------
L_855F: jmp     L_8591                          ; 855F

; ----------------------------------------------------------------------------
L_8562: lda     #$80                            ; 8562
        sta     LoadedObj_CollisionBox_HalfWidth; 8564
        lda     #$80                            ; 8566
        sta     LoadedObj_CollisionBox_HalfHeight; 8568
        lda     LoadedObj + Obj::Scratch1       ; 856A
        bne     L_8580                          ; 856C
        jsr     LD7E3                           ; 856E
        lda     #$06                            ; 8571
        sta     $D4                             ; 8573
        lda     $C5                             ; 8575
        ora     #$08                            ; 8577
        sta     $C5                             ; 8579
        lda     #$51                            ; 857B
        jsr     Enqueue_Sound_Command           ; 857D
L_8580: inc     LoadedObj + Obj::Scratch1       ; 8580
        bne     L_8586                          ; 8582
        dec     LoadedObj + Obj::Scratch1       ; 8584
L_8586: lda     LoadedObj + Obj::Scratch1       ; 8586
        cmp     #$90                            ; 8588
        bne     L_8591                          ; 858A
        lda     #$02                            ; 858C
        jsr     LD1EF                           ; 858E
L_8591: lda     #$10                            ; 8591
        sta     $40                             ; 8593
        lda     #$10                            ; 8595
        sta     $41                             ; 8597
        jsr     ScreenPos_Compute               ; 8599
        lda     LoadedObj + Obj::Scratch1       ; 859C
        lsr     a                               ; 859E
        lsr     a                               ; 859F
        tax                                     ; 85A0
        cpx     #$18                            ; 85A1
        bcc     L_85A7                          ; 85A3
        ldx     #$17                            ; 85A5
L_85A7: lda     L_85CA,x                        ; 85A7
        tax                                     ; 85AA
        lda     L_85BE,x                        ; 85AB
        sta     $45                             ; 85AE
        lda     L_85C4,x                        ; 85B0
        sta     $44                             ; 85B3
        jsr     LEDF5                           ; 85B5
        jsr     LEC73                           ; 85B8
        jmp     L_88E9                          ; 85BB

; ----------------------------------------------------------------------------
L_85BE: .byte   $82,$80,$82,$80,$84,$86         ; 85BE
L_85C4: .byte   $C0,$C0,$00,$00,$00,$00         ; 85C4
L_85CA: .byte   $03,$00,$01,$02,$03,$00,$01,$02 ; 85CA
        .byte   $03,$00,$01,$02,$03,$03,$03,$03 ; 85D2
        .byte   $03,$03,$03,$04,$05,$05,$05,$05 ; 85DA
; ----------------------------------------------------------------------------
L_85E2: lda     $4F                             ; 85E2
        bne     L_8621                          ; 85E4
        lda     LoadedObj + Obj::Health         ; 85E6
        beq     L_8621                          ; 85E8
        lda     #$7F                            ; 85EA
        jsr     LD7A0                           ; 85EC
        bmi     L_85FB                          ; 85EF
        jsr     LD2AB                           ; 85F1
        jsr     LD256                           ; 85F4
        bne     L_8620                          ; 85F7
        lda     #$40                            ; 85F9
L_85FB: and     #$7F                            ; 85FB
        beq     L_8620                          ; 85FD
        pha                                     ; 85FF
        jsr     LD136                           ; 8600
        pla                                     ; 8603
        asl     a                               ; 8604
        eor     #$FF                            ; 8605
        clc                                     ; 8607
        adc     #$01                            ; 8608
        clc                                     ; 860A
        adc     LoadedObj + Obj::Health         ; 860B
        bcs     L_8611                          ; 860D
        lda     #$00                            ; 860F
L_8611: sta     LoadedObj + Obj::Health         ; 8611
        lda     #$01                            ; 8613
        sta     LoadedObj + Obj::Scratch0       ; 8615
        lda     #$20                            ; 8617
        sta     $4F                             ; 8619
        lda     #$33                            ; 861B
        jsr     Enqueue_Sound_Command           ; 861D
L_8620: rts                                     ; 8620

; ----------------------------------------------------------------------------
L_8621: jmp     LD790                           ; 8621

; ----------------------------------------------------------------------------
L_8624: lda     $F3                             ; 8624
        and     #$20                            ; 8626
        beq     L_8676                          ; 8628
        lda     $03D4                           ; 862A
        cmp     $14                             ; 862D
        bne     L_8676                          ; 862F
        lda     LoadedObj + Obj::Position_X_Lo  ; 8631
        sta     $00                             ; 8633
        lda     LoadedObj + Obj::Position_X_Hi  ; 8635
        sta     $01                             ; 8637
        sec                                     ; 8639
        lda     $00                             ; 863A
        sbc     #$80                            ; 863C
        sta     $00                             ; 863E
        bcs     L_8644                          ; 8640
        dec     $01                             ; 8642
L_8644: lda     $03D0                           ; 8644
        cmp     $00                             ; 8647
        lda     $03D1                           ; 8649
        sbc     $01                             ; 864C
        asl     a                               ; 864E
        bne     L_8676                          ; 864F
        lda     $03D3                           ; 8651
        sec                                     ; 8654
        sbc     LoadedObj + Obj::Position_Y_Hi  ; 8655
        asl     a                               ; 8657
        bne     L_8676                          ; 8658
        lda     #$1D                            ; 865A
        sta     LoadedObj + Obj::Type           ; 865C
        lda     $03D0                           ; 865E
        sta     LoadedObj + Obj::Position_X_Lo  ; 8661
        lda     $03D1                           ; 8663
        sta     LoadedObj + Obj::Position_X_Hi  ; 8666
        lda     #$16                            ; 8668
        sta     $03D6                           ; 866A
        lda     #$E4                            ; 866D
        sta     LoadedObj + Obj::Velocity_Y     ; 866F
        lda     #$3E                            ; 8671
        jsr     Enqueue_Sound_Command           ; 8673
L_8676: rts                                     ; 8676

; ----------------------------------------------------------------------------
L_8677: jmp     L_86CC                          ; 8677

; ----------------------------------------------------------------------------
L_867A: lda     #$40                            ; 867A
        sta     LoadedObj_CollisionBox_HalfWidth; 867C
        lda     #$80                            ; 867E
        sta     LoadedObj_CollisionBox_HalfHeight; 8680
        inc     LoadedObj + Obj::Velocity_Y     ; 8682
        inc     LoadedObj + Obj::Velocity_Y     ; 8684
        jsr     LD3DE                           ; 8686
        lda     $03D6                           ; 8689
L_868C: bne     L_86CC                          ; 868C
        lda     #$03                            ; 868E
        sta     LoadedObj + Obj::Type           ; 8690
        lda     $03FF                           ; 8692
        sta     LoadedObj + Obj::Health         ; 8695
        lda     #$00                            ; 8697
        sta     LoadedObj + Obj::Velocity_X     ; 8699
        sta     LoadedObj + Obj::Velocity_Y     ; 869B
        lda     $03D0                           ; 869D
        sta     LoadedObj + Obj::Position_X_Lo  ; 86A0
        lda     $03D1                           ; 86A2
        sta     LoadedObj + Obj::Position_X_Hi  ; 86A5
        lda     $03D2                           ; 86A7
        sta     LoadedObj + Obj::Position_Y_Lo  ; 86AA
        lda     $03D3                           ; 86AC
        sta     LoadedObj + Obj::Position_Y_Hi  ; 86AF
        lda     $03D5                           ; 86B1
        sta     LoadedObj + Obj::Facing         ; 86B4
        jsr     LD2B9                           ; 86B6
        lda     #$18                            ; 86B9
        sta     $40                             ; 86BB
        lda     #$10                            ; 86BD
        sta     $41                             ; 86BF
        jsr     ScreenPos_Compute               ; 86C1
        jsr     L_950D                          ; 86C4
        lda     #$FF                            ; 86C7
        sta     $C1                             ; 86C9
        rts                                     ; 86CB

; ----------------------------------------------------------------------------
L_86CC: lda     #$08                            ; 86CC
        sta     $40                             ; 86CE
        lda     #$10                            ; 86D0
        sta     $41                             ; 86D2
        jsr     ScreenPos_Compute               ; 86D4
        jsr     LCBDF                           ; 86D7
        jsr     LD790                           ; 86DA
        lda     LoadedObj + Obj::Facing         ; 86DD
        eor     #$80                            ; 86DF
        and     #$80                            ; 86E1
        lsr     a                               ; 86E3
        sta     $44                             ; 86E4
        lda     #$61                            ; 86E6
        jsr     MetaSprite_Render               ; 86E8
        jmp     L_88E9                          ; 86EB

; ----------------------------------------------------------------------------
L_86EE: lda     $1C                             ; 86EE
        clc                                     ; 86F0
        adc     #$20                            ; 86F1
        and     #$C0                            ; 86F3
        sta     $1C                             ; 86F5
        lda     $1E                             ; 86F7
        clc                                     ; 86F9
        adc     #$20                            ; 86FA
        and     #$C0                            ; 86FC
        sta     $1E                             ; 86FE
        lda     #$00                            ; 8700
        sta     LoadedObj + Obj::Scratch1       ; 8702
        sta     $4F                             ; 8704
        sta     $D9                             ; 8706
        lda     #$1E                            ; 8708
        ldx     LoadedObj + Obj::Facing         ; 870A
        bpl     L_8710                          ; 870C
        lda     #$21                            ; 870E
L_8710: sta     LoadedObj + Obj::Type           ; 8710
        rts                                     ; 8712

; ----------------------------------------------------------------------------
L_8713: jmp     L_8737                          ; 8713

; ----------------------------------------------------------------------------
L_8716: inc     LoadedObj + Obj::Scratch1       ; 8716
        lda     LoadedObj + Obj::Scratch1       ; 8718
        cmp     #$1C                            ; 871A
        bne     L_8737                          ; 871C
        clc                                     ; 871E
        lda     LoadedObj + Obj::Position_X_Lo  ; 871F
        adc     #$80                            ; 8721
        sta     LoadedObj + Obj::Position_X_Lo  ; 8723
        lda     LoadedObj + Obj::Position_X_Hi  ; 8725
        adc     #$08                            ; 8727
        sta     LoadedObj + Obj::Position_X_Hi  ; 8729
        lda     LoadedObj + Obj::TileIndex      ; 872B
        clc                                     ; 872D
        adc     #$09                            ; 872E
        sta     LoadedObj + Obj::TileIndex      ; 8730
        inc     LoadedObj + Obj::Type           ; 8732
        jmp     L_8740                          ; 8734

; ----------------------------------------------------------------------------
L_8737: jsr     LD790                           ; 8737
        jmp     L_87DA                          ; 873A

; ----------------------------------------------------------------------------
L_873D: jmp     L_874A                          ; 873D

; ----------------------------------------------------------------------------
L_8740: jsr     LCD28                           ; 8740
        bne     L_8747                          ; 8743
        inc     LoadedObj + Obj::Type           ; 8745
L_8747: jsr     LD1AD                           ; 8747
L_874A: jmp     L_88E9                          ; 874A

; ----------------------------------------------------------------------------
L_874D: jmp     L_875B                          ; 874D

; ----------------------------------------------------------------------------
L_8750: dec     LoadedObj + Obj::Scratch1       ; 8750
        bne     L_875B                          ; 8752
        lda     #$1B                            ; 8754
        sta     LoadedObj + Obj::Type           ; 8756
        jsr     LC5B2                           ; 8758
L_875B: jmp     L_87A9                          ; 875B

; ----------------------------------------------------------------------------
L_875E: jmp     L_8782                          ; 875E

; ----------------------------------------------------------------------------
L_8761: inc     LoadedObj + Obj::Scratch1       ; 8761
        lda     LoadedObj + Obj::Scratch1       ; 8763
        cmp     #$1C                            ; 8765
        bne     L_8782                          ; 8767
        sec                                     ; 8769
        lda     LoadedObj + Obj::Position_X_Lo  ; 876A
        sbc     #$80                            ; 876C
        sta     LoadedObj + Obj::Position_X_Lo  ; 876E
        lda     LoadedObj + Obj::Position_X_Hi  ; 8770
        sbc     #$08                            ; 8772
        sta     LoadedObj + Obj::Position_X_Hi  ; 8774
        lda     LoadedObj + Obj::TileIndex      ; 8776
        sec                                     ; 8778
        sbc     #$09                            ; 8779
        sta     LoadedObj + Obj::TileIndex      ; 877B
        inc     LoadedObj + Obj::Type           ; 877D
        jmp     L_878B                          ; 877F

; ----------------------------------------------------------------------------
L_8782: jsr     LD790                           ; 8782
        jmp     L_87A9                          ; 8785

; ----------------------------------------------------------------------------
L_8788: jmp     L_8795                          ; 8788

; ----------------------------------------------------------------------------
L_878B: jsr     LCD28                           ; 878B
        bne     L_8792                          ; 878E
        inc     LoadedObj + Obj::Type           ; 8790
L_8792: jsr     LD1AD                           ; 8792
L_8795: jmp     L_88E9                          ; 8795

; ----------------------------------------------------------------------------
L_8798: jmp     L_87A6                          ; 8798

; ----------------------------------------------------------------------------
L_879B: dec     LoadedObj + Obj::Scratch1       ; 879B
        bne     L_87A6                          ; 879D
        lda     #$1B                            ; 879F
        sta     LoadedObj + Obj::Type           ; 87A1
        jsr     LC5B2                           ; 87A3
L_87A6: jmp     L_87DA                          ; 87A6

; ----------------------------------------------------------------------------
L_87A9: lda     #$08                            ; 87A9
        sta     $40                             ; 87AB
        lda     #$10                            ; 87AD
        sta     $41                             ; 87AF
        jsr     ScreenPos_Compute               ; 87B1
        lda     $3F                             ; 87B4
        pha                                     ; 87B6
        lda     $3E                             ; 87B7
        pha                                     ; 87B9
        lda     $3E                             ; 87BA
        sec                                     ; 87BC
        sbc     #$28                            ; 87BD
        sta     $3E                             ; 87BF
        lda     #$00                            ; 87C1
        sta     $44                             ; 87C3
        lda     #$31                            ; 87C5
        jsr     MetaSprite_Render               ; 87C7
        pla                                     ; 87CA
        sta     $3E                             ; 87CB
        pla                                     ; 87CD
        sta     $3F                             ; 87CE
        lda     $3E                             ; 87D0
        sec                                     ; 87D2
        sbc     LoadedObj + Obj::Scratch1       ; 87D3
        sta     $3E                             ; 87D5
        jmp     L_84B6                          ; 87D7

; ----------------------------------------------------------------------------
L_87DA: lda     #$08                            ; 87DA
        sta     $40                             ; 87DC
        lda     #$10                            ; 87DE
        sta     $41                             ; 87E0
        jsr     ScreenPos_Compute               ; 87E2
        lda     $3F                             ; 87E5
        pha                                     ; 87E7
        lda     $3E                             ; 87E8
        pha                                     ; 87EA
        lda     $3E                             ; 87EB
        clc                                     ; 87ED
        adc     #$18                            ; 87EE
        sta     $3E                             ; 87F0
        lda     #$00                            ; 87F2
        sta     $44                             ; 87F4
        lda     #$31                            ; 87F6
        jsr     MetaSprite_Render               ; 87F8
        pla                                     ; 87FB
        sta     $3E                             ; 87FC
        pla                                     ; 87FE
        sta     $3F                             ; 87FF
        lda     $3E                             ; 8801
        clc                                     ; 8803
        adc     LoadedObj + Obj::Scratch1       ; 8804
        sta     $3E                             ; 8806
        jmp     L_84B6                          ; 8808

; ----------------------------------------------------------------------------
L_880B: lda     $F3                             ; 880B
        and     #$40                            ; 880D
        beq     L_8829                          ; 880F
        lda     #$1C                            ; 8811
        sta     $00                             ; 8813
        ldx     #$0E                            ; 8815
        jsr     FindEmptyObjectSlot             ; 8817
        beq     L_8829                          ; 881A
        lda     #$2A                            ; 881C
        sta     ObjectTable + Obj::Type,x       ; 881E
        jsr     LD7C0                           ; 8821
        lda     #$34                            ; 8824
        jsr     Enqueue_Sound_Command           ; 8826
L_8829: rts                                     ; 8829

; ----------------------------------------------------------------------------
L_882A: nop                                     ; 882A
        nop                                     ; 882B
        nop                                     ; 882C
L_882D: ldx     #$30                            ; 882D
        lda     LoadedObj + Obj::Facing         ; 882F
        bpl     L_8835                          ; 8831
        ldx     #$D0                            ; 8833
L_8835: stx     LoadedObj + Obj::Velocity_X     ; 8835
        lda     #$00                            ; 8837
        sta     LoadedObj + Obj::Velocity_Y     ; 8839
        clc                                     ; 883B
        lda     LoadedObj + Obj::Position_Y_Lo  ; 883C
        adc     #$20                            ; 883E
        sta     LoadedObj + Obj::Position_Y_Lo  ; 8840
        bcc     L_8846                          ; 8842
        inc     LoadedObj + Obj::Position_Y_Hi  ; 8844
L_8846: jsr     LD2B9                           ; 8846
        lda     #$20                            ; 8849
        sta     LoadedObj + Obj::Scratch1       ; 884B
        inc     LoadedObj + Obj::Type           ; 884D
        rts                                     ; 884F

; ----------------------------------------------------------------------------
L_8850: jmp     L_8867                          ; 8850

; ----------------------------------------------------------------------------
L_8853: lda     #$80                            ; 8853
        sta     LoadedObj_CollisionBox_HalfWidth; 8855
        lda     #$80                            ; 8857
        sta     LoadedObj_CollisionBox_HalfHeight; 8859
        jsr     LD2DE                           ; 885B
        jsr     LD2AB                           ; 885E
        bmi     L_8886                          ; 8861
        dec     LoadedObj + Obj::Scratch1       ; 8863
        beq     L_8886                          ; 8865
L_8867: lda     #$10                            ; 8867
        sta     $40                             ; 8869
        lda     #$10                            ; 886B
        sta     $41                             ; 886D
        jsr     ScreenPos_Compute               ; 886F
        bne     L_8889                          ; 8872
        lda     #$01                            ; 8874
        jsr     LD7A0                           ; 8876
        bmi     L_8886                          ; 8879
        lda     #$00                            ; 887B
        sta     $44                             ; 887D
        lda     #$25                            ; 887F
        sta     $45                             ; 8881
        jmp     LECB4                           ; 8883

; ----------------------------------------------------------------------------
L_8886: jsr     L_9B81                          ; 8886
L_8889: jmp     LD82C                           ; 8889

.endmacro

