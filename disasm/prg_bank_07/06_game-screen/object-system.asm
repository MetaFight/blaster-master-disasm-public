.macro MAC_06_game_screen__object_system_1_of_8
; ----------------------------------------------------------------------------
...
; If Velocity_X is positive, use sign-extension byte $00, otherwise use $FF
        bpl     _OnSignExtensionByteChosen      ; D32B
        lda     #$FF                            ; D32D
_OnSignExtensionByteChosen:
        pha                                     ; D32F
        txa                                     ; D330
...
.macro MAC_06_game_screen__object_system_3_of_8
; ----------------------------------------------------------------------------
...
.proc Obj_TryDamagePlayer
; Stash A (contact damage to deal) into WR_44__OAM_Attribute__or__Outgoing_Contact_Damage.
        sta     $44                             ; D71F
        lda     $40                             ; D721
        lsr     a                               ; D723
; $00 = hitbox_width / 2
...
.macro MAC_06_game_screen__object_system_5_of_8
; ----------------------------------------------------------------------------
...
;     A = 0
.proc Obj_TryCloneIntoEmptySlot
        lda     #$D2                            ; DF0F
        sta     L0000                           ; DF11
        ldx     #$70                            ; DF13
...
; past the slot.
_CopyRecord:
        ::_Obj_TryCloneIntoEmptySlot__CopyRecord = _CopyRecord
        ldy     #$00                            ; DF27
...
;   on failure,
;     A = #$00
.proc Obj_TrySpawnChild_A0_Throttled
...
; 
; The child's ObjType is taken from SpawnChild_ObjType.  The remaining fields are cloned from the
; parent.
; 
; Itput:
;   WR_Context_Dependent_00 = TBD
;   SpawnChild_ObjType = The child's Object Type
; 
; Output:
;   on success,
;     A = $FF
...
.proc Obj_GravityMoveBounce_Double
        ldy     #$00                            ; DFA0
        pha                                     ; DFA2
        tya                                     ; DFA3
        and     Global_FrameCounter             ; DFA4
; This always branches.
        beq     _ApplyGravity                   ; DFA6
; dead
        pla                                     ; DFA8
; dead
        jmp     _Move                           ; DFA9
...
; ----------------------------------------------------------------------------
; Applies a gravity (of 2 px/f^2) and terrain collision+bouncing (via
; Obj_GravityMoveBounce_Double).
; 
; If a vertical collision happened while travelling downward the object is deemed to have 'landed'
; and its Velocity_Y is cleared.
; 
; Returns:
;   on landing,
;     Velocity_Y = 0
...
.proc Obj_MoveBounce_TurnAtLedge
...
.proc Obj_SetOAMAttr_FlipX_and_Palette
...
.macro MAC_06_game_screen__object_system_6_of_8
; ----------------------------------------------------------------------------
...
.proc Obj_MoveAndCollide_Double
...
.proc Obj_ReflectHeading
        lda     TerrainCollisionFlags           ; E0D0
        bmi     _HandleWall                     ; E0D2
        asl     a                               ; E0D4
        bmi     _FloorCeiling                   ; E0D5
        rts                                     ; E0D7

; ----------------------------------------------------------------------------
; LoadedObj.Facing = $80 - LoadedObj.Facing.
_HandleWall:
        ::_Obj_ReflectHeading__HandleWall = _HandleWall
...
_FloorCeiling:
        ::_Obj_ReflectHeading__FloorCeiling = _FloorCeiling
        lda     #$00                            ; E0E5
        sec                                     ; E0E7
...
.macro MAC_06_game_screen__object_system_7_of_8
; ----------------------------------------------------------------------------
...
.macro MAC_06_game_screen__object_system_8_of_8
; ----------------------------------------------------------------------------
