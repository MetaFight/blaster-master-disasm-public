...
A finer-grained view of M4 above, broken down by the same organizational grouping the disassembly's own bank-splitting uses (e.g. bank 06's `object-handlers/hopper_6hp`). `%` counts both top-level subroutines and their interior sublabels, which M4 does not — a sublabel has no verification status of its own, so it inherits its enclosing subroutine's. A note on a row flags a verified routine that directly calls a still-unverified subroutine — a caveat on `%`, not a deduction from it (a routine's own logic was reviewed; part of its behaviour still depends on code nobody has confirmed yet).
...
| 06 | `object-handlers/hopper_6hp` | 94% | ObjHandler_Tank_5F_Gray_Hopper_6HP_Init, ObjHandler_Tank_60_Gray_Hopper_6HP_Attacking depend on 10 unverified routines. | ⬜⬜⬜◽ |
...
_Generated: 2026-07-29 10:27 UTC_
